...
/**
 * Created by IntelliJ IDEA. User: exprmntr Date: 11/18/11 Time: 3:21 PM
 * 
 * @author Anton Ohitin, Vladislav Grechko
 */
...
public class ImgUtils {
	public static String get(String url, int id) throws IOException {
		...
		new File(Consts.MediaPath).mkdirs();		
	...
	public static void makeThumb(int id, String ext) throws IOException {
		BufferedImage img = ImageIO.read(new File(Consts.MediaPath
				+ ((Integer) id).toString() + "." + ext));
		
		new File(Consts.MediaPath + "thumb\\").mkdirs();
		ImageIO.write(getThumbnail(img, 150, 150), "jpg", new File(
				Consts.MediaPath + "thumb\\" +
						"" + ((Integer) id).toString()
						+ ".jpg"));
	}
}
